#!/usr/bin/env python3
"""
Fetch pro matches from last 6 months from Stratz API
Includes: heroes, roles, tier, league info
...
class StratzAPIClient:
    ...
    def fetch_pro_match_ids(self, start_time: int, end_time: int, tier: str, skip: int = 0, take: int = 100) -> List[int]:
        """Fetch pro match IDs for a given tier and time range"""
        
        key_idx = self.wait_for_available_key()
        
        # Query to get match IDs from leagues
        query = """
        query GetProMatches($request: LeagueRequestType!) {
        ...
        try:
            ...
            if "data" in data and "leagues" in data["data"]:
                for league in data["data"]["leagues"]:
                    if league and "matches" in league:
    ...
    def fetch_match_details(self, match_id: int) -> Optional[Dict]:
        """Fetch full match details including heroes, roles, league"""
        
        key_idx = self.wait_for_available_key()
        
        query = """
        query GetMatchDetails($id: Long!) {
            match(id: $id) {
                id
                didRadiantWin
                leagueId
...
def main():
    ...
    print("=" * 70)
    print("Fetch Recent Pro Matches from Stratz (Last 6 Months)")
    print("=" * 70)
    ...
    # First, get all match IDs from pro leagues
    all_match_ids = set()
    
    for tier in PRO_TIERS:
        print(f"   Fetching {tier} matches...")
        
        skip = 0
        take = 100
        while True:
            match_ids = api_client.fetch_pro_match_ids(START_TIME, END_TIME, tier, skip, take)
            
            if not match_ids:
                break
            
            all_match_ids.update(match_ids)
            print(f"     Found {len(match_ids)} matches (total: {len(all_match_ids)})")
    ...
    for i, match_id in enumerate(match_ids_list):
        match_details = api_client.fetch_match_details(match_id)
        
        if match_details:
            parsed_data = parse_match_data(match_details)
            all_matches[str(match_id)] = parsed_data
            successful += 1
        ...
        if processed % 100 == 0:
            ...
            print(f"   Progress: {processed}/{total_matches} ({percent:.1f}%) | {successful} successful")
    
    print(f"\n✅ Fetched details for {successful}/{total_matches} matches")
